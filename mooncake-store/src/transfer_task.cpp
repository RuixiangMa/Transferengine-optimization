...
#ifdef USE_TENT
    if (tent_engine_) tent_engine_->freeBatch(batch_id_);
    else
#endif
...
void TransferEngineOperationState::check_task_status() {
    ...
    for (size_t i = 0; i < batch_size_; ++i) {
        ...
        if (tent_engine_) {
            ...
            auto tent_s = tent_engine_->getTransferStatus(batch_id_, i, tent_status);
            if (!tent_s.ok()) {
        ...
        if (op_code == TransferRequest::READ) {
            ...
            switch (strategy) {
                ...
                case TransferStrategy::TRANSFER_ENGINE:
                    future =
                        submitTransferEngineOperation(handle, slices, op_code, 0, intent);
                    break;
                default:
...
std::optional<TransferFuture> TransferSubmitter::submit_batch(
    ...
    for (size_t i = 0; i < replicas.size(); ++i) {
        ...
        if (tent_engine_) {
            ...
            auto status = tent_engine_->openSegment(seg_id, handle.transport_endpoint_);
            if (!status.ok()) {
...
TransferSubmitter::submit_batch_get_offload_object(
    ...
    if (!use_local_memcpy) {
        ...
        if (tent_engine_) {
            tent::SegmentID seg_id;
            auto status = tent_engine_->openSegment(seg_id, transfer_engine_addr);
            if (!status.ok()) {
...
std::optional<TransferFuture> TransferSubmitter::submitTransfer(
    ...
    if (tent_engine_) {
        ...
        for (auto& r : requests) {
            ...
            tr.length = r.length;
            tr.transport_hint = mooncake::tent::c_to_transport_hint(r.transport_hint);
            tr.intent_type = intent;
...
std::optional<TransferFuture> TransferSubmitter::submitMemoryReadOperation(
    ...
    if (strategy == TransferStrategy::TRANSFER_ENGINE) {
        return submitTransferEngineOperation(handle, slices,
                                             TransferRequest::READ, src_offset, intent);
    }
